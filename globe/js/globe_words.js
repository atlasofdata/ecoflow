...
var count = 1,
  ...
  pairwize = [],
  pileups = [],
  group_mesh = new THREE.Group(),
  group_links = new THREE.Group(),
  group_pileups = new THREE.Group();
...
function readJSON(data) {
  ...
  for (var i = 0; i < n_words; i++)
    words.push(data_graph_pubmed["keywords"][i]["id"]);
  word = words[i_word];
  console.log(words);
  ...
  for (var w = 0; w < n_words; w++) {
    for (var i = 0; i < pairwize[w]; i++)
      min_time = Math.min(
        min_time,
        data_graph_pubmed[words[w]]["links"][i]["time"]
      );
  }
  time = min_time;
  ...
  for (var i = 0; i < n_countries; i++) {
    LATi =
      0.5 * Math.PI - (data_graph_pubmed["LL"][i]["lat"] * Math.PI) / 180.0;
    LONi =
      0.5 * Math.PI + (data_graph_pubmed["LL"][i]["lon"] * Math.PI) / 180.0; // I do not understand the 0.5*Math.PI but at least it works
    ...
    if (i >= n_countries - 2)
      country = new THREE.SphereGeometry(1.0, segments0, rings0);
    else country = new THREE.SphereGeometry(radius0, segments0, radius0);
    if (i === n_countries - 2)
      material_country = new THREE.MeshBasicMaterial({ color: 0x0000ff });
    else {
      if (i === n_countries - 1)
        material_country = new THREE.MeshBasicMaterial({ color: 0x00ff00 });
  ...
  for (var i = 0; i < pairwize[i_word]; i++)
    min_weight = Math.min(
      min_weight,
      data_graph_pubmed[word]["links"][i]["value"]
    );
  ...
  for (var i = 0; i < pairwize[i_word]; i++) {
    if (data_graph_pubmed[word]["links"][i]["time"] === time) {
      ...
      source =
        data_graph_pubmed["LL"][data_graph_pubmed[word]["links"][i]["source"]];
      target =
        data_graph_pubmed["LL"][data_graph_pubmed[word]["links"][i]["target"]];
      ...
      weighti =
        Math.log(data_graph_pubmed[word]["links"][i]["value"]) / Math.log(2.0) +
        0.1;
      // weighti=1.0;
      // mid-point
      ...
      j = j + 1;
      source =
        data_graph_pubmed["nodes"][
          data_graph_pubmed[word]["links"][i]["source"]
        ]["id"];
      target =
        data_graph_pubmed["nodes"][
          data_graph_pubmed[word]["links"][i]["target"]
        ]["id"];
  ...
  for (var i = 0; i < pileups[i_word]; i++) {
    if (data_graph_pubmed[word]["pileups"][i]["time"] === time) {
      // Starting and target points
      source =
        data_graph_pubmed["LL"][
          data_graph_pubmed[word]["pileups"][i]["country"]
        ];
      LATi = 0.5 * Math.PI - (source["lat"] * Math.PI) / 180.0;
      LONi = 0.5 * Math.PI + (source["lon"] * Math.PI) / 180.0;
      vS = new THREE.Vector3(
      ...
      weighti =
        Math.log(data_graph_pubmed[word]["pileups"][i]["value"]) /
        Math.log(2.0);
      vT = new THREE.Vector3(
        Math.sin(LATi) * Math.sin(LONi),
        Math.cos(LATi),
        Math.sin(LATi) * Math.cos(LONi)
      ).multiplyScalar(radius * (1.0 + pileup_weight * weighti));
      // Lines
  ...
  document.getElementById("yearm1").addEventListener(
    ...
    function() {
      ...
      document.getElementById(
        ...
        temp_anom_land["data"][time]
      );
      document.getElementById(
        "tanom_ocean"
      ).innerHTML = "temperature anomaly ocean : ".concat(
        temp_anom_ocean["data"][time]
      );
  ...
  document.getElementById("pkeyword").addEventListener(
    "click",
    function() {
      i_word -= 1;
  ...
  document.getElementById("nkeyword").addEventListener(
    "click",
    function() {
      i_word += 1;
...
function change_time() {
  ...
  loader_font.load(
    ...
    function(font) {
      ...
      mesh_text = new THREE.Mesh(
        new THREE.TextGeometry(time.toString(), {
          font: font,
          size: 80,
          height: 5,
          curveSegments: 120,
  ...
  for (var i = 0; i < pileups[i_word]; i++) {
    if (data_graph_pubmed[word]["pileups"][i]["time"] === time) {
      ...
      if (j < group_pileups.children.length) {
        ...
        if (source === "facebook")
          group_pileups.children[j].material = new THREE.LineBasicMaterial({
            color: 0x0000ff
          });
        if (source === "google")
          group_pileups.children[j].material = new THREE.LineBasicMaterial({
